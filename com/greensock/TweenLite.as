		...
		public function setEnabled(enabled:Boolean, ignoreTimeline:Boolean):Boolean {
			if (enabled && this.gc) {
				...
				if (a) {
					...
					while (--i > -1) {
						if (a[i] == this) {
							a.splice(i, 1); //in case this tween was disabled and then enabled before it was gc'd, we need to make sure instances don't build up in the master list over time (in extremely rare situations this could bog down performance, like if a tween's onComplete restarts it infinitely)
						}
					}
				} else {
